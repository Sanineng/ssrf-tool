...
void writeOutAFLSHM(string PORT){
  ...
  if (afl_shm_loc){
    struct stat info;
    if( stat( "/fs", &info ) == 0 ){
      myfile.open ("/fs/__AFL_SHM_ID"); 
      myfile << afl_shm_loc << endl;
      myfile.close();
    } else {
      myfile.open ("/__AFL_SHM_ID"); 
      myfile << afl_shm_loc << endl;
      myfile.close();
    }

    cout << "RECVD AFL_SHM  _ID of " << hex << afl_shm_loc << endl;
  ...
  if(curl) {
    ...
    headers = NULL;
      cout << "----------------------- HTTP REQUEST --------------------------" << endl;
       ...
       const char* filePath = "/tmp/packet.out";
        std::ofstream outputFile(filePath);
        if (!outputFile.is_open()) {
            std::cerr << "Error in saving Packet : " << filePath << std::endl;
        }

        outputFile << reqD->getRequest();

        outputFile.close();
    std::cout << "Packet Saved :  " << filePath << std::endl;
...
int main(int argc, char *argv[])
{
  ...
  method = getArg(argc, argv, "--method", true);
